...
from holoassist_perception_nodes.utils.math3d import rotation_matrix_from_quaternion
...
class AprilCubeTrackerNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("debug_output_fps", 25.0)
        ...
        self.debug_output_fps        = max(0.0, float(self.get_parameter("debug_output_fps").value))
        ...
        self._rgb_frame_times: deque = deque(maxlen=30)
        self._debug_publish_credit = 1.0
        ...
        self._debug_pub = self.create_publisher(
            Image, "/holoassist/perception/debug_image", reliable_qos
        )
        ...
        self.get_logger().info(
            ...
            f"apriltag_timeout_s={self.apriltag_timeout_s:.2f} debug_output_fps={self.debug_output_fps:.1f}"
        )
    ...
    def _should_publish_debug_frame(self, now_ns: int) -> bool:
        self._rgb_frame_times.append(now_ns)
        if self.debug_output_fps <= 0.0 or len(self._rgb_frame_times) < 2:
            return True
        span_s = (self._rgb_frame_times[-1] - self._rgb_frame_times[0]) / 1e9
        if span_s <= 0.0:
            return True
        input_fps = (len(self._rgb_frame_times) - 1) / span_s
        if input_fps <= self.debug_output_fps + 0.5:
            self._debug_publish_credit = 1.0
            return True
        self._debug_publish_credit += self.debug_output_fps / input_fps
        if self._debug_publish_credit < 1.0:
            return False
        self._debug_publish_credit -= 1.0
        return True
    ...
    def _on_rgb(self, msg: Image) -> None:
        ...
        if not self._should_publish_debug_frame(now.nanoseconds):
            return
        try:
