    ...
    def __init__(self) -> None:
        ...
        self.bin_check_pubs = [
            self.create_publisher(
                String,
                f"/holoassist/perception/april_cube_{idx + 1}_bin_check",
                reliable_qos,
            )
            for idx in range(4)
        ]
        ...
        if not bin_file:
            try:
                ...
                self.get_logger().warn(
                    "bin check: could not locate moveit_robot_control package: %s" % exc
                )
                return {}
        try:
            with open(bin_file, encoding="utf-8") as stream:
                raw = json.load(stream)
            bins = {
                name: np.array(data["xyz"], dtype=np.float64)
                for name, data in raw.items()
            }
            self.get_logger().info(
                "bin check: loaded %d bins from %s (margin %.3f m, frame %s)"
                % (len(bins), bin_file, self._bin_xy_margin_m, self._bin_check_frame)
            )
            return bins
        except Exception as exc:
            self.get_logger().warn("bin check: failed to load bin_poses.json: %s" % exc)
            return {}

    def _run_bin_checks(self) -> None:
        for cube_idx, center in enumerate(self._last_centers):
            if center is not None:
                self._publish_bin_check(cube_idx, center, self._last_output_frames[cube_idx])

    def _publish_bin_check(self, cube_idx: int, center: np.ndarray, workspace_frame: str) -> None:
        if not self._bins:
            return
        if workspace_frame == self._bin_check_frame:
            ...
            try:
                ...
                self._log_throttled(
                    ...
                    self.WARNING_THROTTLE_S,
                    "bin check: TF %s to %s not available" % (workspace_frame, self._bin_check_frame),
                )
                return

            translation = tf_msg.transform.translation
            rotation = tf_msg.transform.rotation
            rot = rotation_matrix_from_quaternion(
                float(rotation.x), float(rotation.y), float(rotation.z), float(rotation.w)
            )
            center_robot = rot @ center + np.array(
                [float(translation.x), float(translation.y), float(translation.z)],
                dtype=np.float64,
            )

        nearest_bin, nearest_dist = min(
            (
                (bin_name, float(np.linalg.norm(center_robot[:2] - bin_pos[:2])))
