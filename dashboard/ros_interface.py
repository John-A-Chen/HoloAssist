...
class RosInterface:
    ...
    def start(self):
        ...
        # Cube perception status (tag IDs + state for bin display)
        for _n in range(1, 5):
            self._node.create_subscription(
                String, f"/holoassist/perception/april_cube_{_n}_status",
                lambda msg, n=_n: self._cube_status_cb(n, msg),
    ...
    # XY bin centres in base_link (from bin_poses.json), with 0.12 m proximity radius
    _BIN_XY = {
        "bin_1": (0.30,  0.00),
        "bin_2": (-0.30,  0.00),
        "bin_3": (0.30, -0.20),
    ...
    def _cube_status_cb(self, cube_id: int, msg):
        s = msg.data
        # Parse visible_tag_ids=[10, 11, ...] from status string
        tags = []
        m = re.search(r"visible_tag_ids=\[([^\]]*)\]", s)
        if m and m.group(1).strip():
            try:
                tags = [int(x) for x in m.group(1).split(",")]
            except ValueError:
                pass

        state_m = re.search(r"\bstate=(\S+)", s)
        state = state_m.group(1) if state_m else "unknown"

        # Parse cube centre position
        bin_name = None
        pos_m = re.search(r"position_m=\(([^)]+)\)", s)
        if pos_m and state == "visible":
            try:
                px, py, _ = [float(v) for v in pos_m.group(1).split(",")]
                for bname, (bx, by) in self._BIN_XY.items():
                    if ((px - bx) ** 2 + (py - by) ** 2) ** 0.5 < self._BIN_RADIUS_M:
                        bin_name = bname
                        break
        ...
        with self._lock:
            self._status.cube_statuses[cube_id] = {
                "tags": tags,
                "state": state,
                "bin": bin_name,
            }
    ...
    def _sample_velocities(self):
        ...
        with self._lock:
            ...
            names = list(self._status.joint_names)
            joint_time = self._status.last_joint_time
        arm_set = set(self._ARM_JOINT_NAMES)
        if vels and names:
            arm_vels = [v for n, v in zip(names, vels)
                        if n in arm_set and not math.isnan(v)]
        else:
            arm_vels = []
        if len(arm_vels) == 6:
            self._velocity_history.append((now, arm_vels))
    ...
    def get_status(self, include_history: bool = True,
                   include_headset: bool = True) -> DashboardStatus:
        ...
        # Snapshot rolling graph data only when requested (GIL-safe deque → list)
        ...
        with self._lock:
            ...
            status = DashboardStatus(
                ...
                headset_jpeg=self._status.headset_jpeg if include_headset else None,
    ...
    def start_calibration_stack(self) -> bool:
        """Spawn easy_handeye2 server + grid_calibration_node as background processes."""
        import pathlib
        if self._calib_procs:
            self._add_event("Calibration stack already running")
            return False

        root = pathlib.Path(__file__).resolve().parent.parent
        demo_ws = root / "ros2_ws" / "install" / "setup.bash"
        ...
        def _run(name: str, cmd: str):
            ...
            self._calib_procs.append((name, proc))
            self._add_event(f"Started calibration process: {name} (pid {proc.pid})")
    ...
    def stop_calibration_stack(self) -> None:
        """Kill calibration stack processes."""
        import os, signal as _sig
        self.calibration_command("stop")
        time.sleep(0.5)
        for name, proc in reversed(self._calib_procs):
            if proc.poll() is None:
                try:
                    os.killpg(os.getpgid(proc.pid), _sig.SIGINT)
                except ProcessLookupError:
                    pass
                self._add_event(f"Stopped calibration process: {name}")
        deadline = time.time() + 6.0
        for _, proc in self._calib_procs:
            try:
                proc.wait(timeout=max(0.1, deadline - time.time()))
            except subprocess.TimeoutExpired:
                proc.kill()
        self._calib_procs.clear()
    ...
    def record_calibration_pose(self) -> int:
        ...
        with self._lock:
            names = list(self._status.joint_names)
            positions = list(self._status.joint_positions)
        pose_map = {n: p for n, p in zip(names, positions)}
        if not all(j in pose_map for j in _ARM):
    ...
    def clear_recorded_cal_poses(self) -> None:
        with self._lock:
            self._status.recorded_cal_poses.clear()
        self._add_event("Cleared recorded calibration poses")

    def calibration_stack_running(self) -> bool:
        self._calib_procs = [(n, p) for n, p in self._calib_procs if p.poll() is None]
        return bool(self._calib_procs)
