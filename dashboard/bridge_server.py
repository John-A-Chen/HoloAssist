...
class BridgeServer:
    HISTORY_INTERVAL = 0.5      # send history arrays at 2 Hz
    HEADSET_INTERVAL = 0.1      # send headset JPEG at 10 Hz
    ...
    def _build_payload(self, include_history: bool,
        ...
        """Build (text_json, headset_bytes|None) off the event loop."""
        status = self.ros.get_status(
            include_history=include_history,
            include_headset=include_headset)
        data = self._serialize(status, include_history)
        text = json.dumps(data)
        headset_frame = None
        if include_headset and status.headset_jpeg:
            headset_frame = bytes([BINARY_HEADSET]) + status.headset_jpeg
        return text, headset_frame
    ...
    async def _push_loop(self):
        ...
        while True:
            if self.clients and not self._sending:
                now = time.time()
                include_history = (now - last_history) >= self.HISTORY_INTERVAL
                include_headset = (now - last_headset) >= self.HEADSET_INTERVAL

                self._sending = True
                try:
                    text, headset_frame = await loop.run_in_executor(
                        None, self._build_payload, include_history,
                        include_headset)

                    if include_history:
                        last_history = now
                    if include_headset and headset_frame:
                        last_headset = now
                    ...
                    await asyncio.gather(
                        *(_send_one(ws) for ws in list(self.clients)))
                    self.clients -= dead
                finally:
                    self._sending = False
    ...
    @staticmethod
    def _serialize(status, include_history: bool = True) -> dict:
        topic_rates = {}
