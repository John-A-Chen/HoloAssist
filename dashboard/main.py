...
class MergedStatusScreen(QWidget):
    ...
    def update_status(self, status):
        # Joint table — filter to arm joints only (skip finger_width / NaN entries)
        arm_set = set(self._ARM_JOINTS)
        arm_rows = [(n, p, v) for n, p, v in zip(
            status.joint_names, status.joint_positions, status.joint_velocities
        ) if n in arm_set]
        for row_i in range(6):
            if row_i < len(arm_rows):
                name, pos, vel = arm_rows[row_i]
                pos_deg = math.degrees(pos)
                self.joint_name_lbls[row_i].setText(name.removesuffix("_joint"))
                self.joint_pos_lbls[row_i].setText(f"{pos_deg:+7.1f}°")
                if math.isnan(vel):
                    self.joint_vel_lbls[row_i].setText("---")
                    self.joint_vel_lbls[row_i].setStyleSheet(f"color: {TEXT_DIM}; border: none;")
                else:
                    abs_vel = abs(vel)
                    vel_color = TEXT if abs_vel < 0.05 else (YELLOW if abs_vel < 0.5 else RED)
                    self.joint_vel_lbls[row_i].setText(f"{vel:+.3f}")
                    self.joint_vel_lbls[row_i].setStyleSheet(f"color: {vel_color}; border: none;")
            else:
                self.joint_name_lbls[row_i].setText(f"joint_{row_i}")
                self.joint_pos_lbls[row_i].setText("---")
                self.joint_vel_lbls[row_i].setText("---")
...
class CalibrationScreen(QWidget):
    ...
    def __init__(self, ros: RosInterface, parent=None):
        ...
        # Row 3: manual pose recording for auto calibration
        rec_row = QHBoxLayout()
    ...
    def _cmd_button(self, text, action):
    ...
    def _on_launch(self):
        import threading
        threading.Thread(target=self.ros.start_calibration_stack, daemon=True).start()

    def _on_shutdown(self):
        import threading
        threading.Thread(target=self.ros.stop_calibration_stack, daemon=True).start()

    def _on_rec_pose(self):
        self.ros.record_calibration_pose()

    def _on_save_poses(self):
        path = self.ros.save_recorded_cal_poses()
        if path:
            self.rec_label.setStyleSheet(f"color: {GREEN};")
            self.rec_label.setText(f"Saved → {path.split('/')[-1]}")

    def _on_clear_poses(self):
        self.ros.clear_recorded_cal_poses()
    ...
    def update_status(self, status):
        ...
        n_rec = len(getattr(status, "recorded_cal_poses", []))
        self.rec_label.setText(f"Recorded: {n_rec} poses")
        if n_rec > 0:
            self.rec_label.setStyleSheet(f"color: {YELLOW if n_rec < 8 else GREEN};")
        else:
            self.rec_label.setStyleSheet(f"color: {TEXT_DIM};")
        self.save_poses_btn.setEnabled(n_rec > 0)
        self.clear_poses_btn.setEnabled(n_rec > 0)

        stack_up = self.ros.calibration_stack_running()
        self.launch_btn.setEnabled(not stack_up)
        self.launch_btn.setStyleSheet(
            f"background-color: {DARK_BG}; color: {TEXT_DIM}; border: 1px solid {BORDER};"
            if stack_up else
            f"background-color: {BLUE}; color: white; border: none;"
        )
        self.shutdown_btn.setEnabled(stack_up)
...
class MainWindow(QMainWindow):
    POLL_INTERVAL_MS = 40  # 25 Hz UI updates
    ...
    def __init__(self, ros: RosInterface):
        ...
        self._active_mode = None   # dim until user explicitly clicks a mode
    ...
    def _poll(self):
        ...
        self._set_calibration_tab_visible(status.operating_mode == "MOVEIT")
    ...
    def _update_mode_buttons(self):
        ...
        if self._active_mode == "TELEOP":
            ...
            # No mode selected yet — both buttons dim
            self.teleop_btn.setStyleSheet(dim_style)
            self.moveit_btn.setStyleSheet(dim_style)
